...
const makeGradient = (text: string, startHex: string, endHex: string) => {
  if (!text) return "";
...
async function startTimer(): Promise<void> {
  ...
  p.intro(
    makeMultilineGradient("🍅  F O C U S  F L O W  🍅", "#ff9a9e", "#fad0c4"),
  );
  ...
  const session = project as SessionData;
  ...
  const formatBar = (percent: number) => {
    ...
    return (
      pc.green("█".repeat(filled)) + pc.gray("░".repeat(barLength - filled))
    );
  };

  p.log.info(
    makeGradient(
      `Starting: ${session.task} (${session.duration} min)`,
      "#89f7fe",
      "#66a6ff",
    ),
  );
  ...
  while (remaining > 0) {
    ...
    process.stdout.write(
      `\r${formatBar(percent)}  ${pc.bold(timeStr)}  ${
        remaining <= 10 ? pc.red("⚡ final sprint") : ""
      }`,
    );
...
(async () => {
  ...
  if (args.includes("history")) await showHistory();
